...
// MARK: - Enabled marker ids (edit here)
// 这里决定“哪些 marker id 会被输出到 Swift”。
// 按需改这个列表即可：例如只想要 0 和 4，就写 {0, 4}
static const std::unordered_set<int> kEnabledMarkerIds = {0, 1, 2, 3, 4, 5};
...
+ (NSArray<MarkerPoseObjC *> *)detectMarkers:(CVPixelBufferRef)pixelBuffer
    ...
    // Filter markers by enabled ID list (see kEnabledMarkerIds above)
    std::vector<int> filteredIds;
    std::vector<std::vector<cv::Point2f>> filteredCorners;
    for (size_t i = 0; i < ids.size(); ++i) {
        if (kEnabledMarkerIds.find(ids[i]) != kEnabledMarkerIds.end()) {
