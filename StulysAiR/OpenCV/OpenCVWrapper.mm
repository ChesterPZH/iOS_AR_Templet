...
+ (NSArray<MarkerPoseObjC *> *)detectMarkers:(CVPixelBufferRef)pixelBuffer
    ...
    // Initialize ArUco dictionary for marker detection
    static cv::Ptr<cv::aruco::Dictionary> dict = cv::makePtr<cv::aruco::Dictionary>(
    ...
    cv::aruco::estimatePoseSingleMarkers(
        ...
        markerSize,
        simdToCv(K),
        cv::Mat::zeros(1,5,CV_64F),  // No distortion
        rvecs,
        tvecs
    );
